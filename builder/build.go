...
func runBuild(args *docopt.Args) error {
	...
	if keys := args.String["--tuf-keys"]; keys != "" {
		if err := json.Unmarshal([]byte(keys), manifest.TUFConfig.RootKeys); err != nil {
			return fmt.Errorf("error parsing --tuf-keys: %s", err)
		}
	}
...
// GetLayer gets a layer either from the local /var/lib/flynn/layer-cache
// directory or from the TUF repository
func (b *Builder) GetLayer(name, id string) (*ct.ImageLayer, error) {
...
func (b *Builder) generateLayerID(name string, run []string, env map[string]string, artifact *ct.Artifact, inputs ...string) (id string, err error) {
	...
	// hash on the layer name
	h.Write([]byte(name))

	// hash on the list of commands being run
	if len(run) > 0 {
		h.Write([]byte(strings.Join(run, "")))
	}

	// hash on the sorted list of environment variables
	if len(env) > 0 {
		sortEnv := make([]string, 0, len(env))
		for k, v := range env {
			sortEnv = append(sortEnv, fmt.Sprintf("%s=%s", k, v))
		}
		sort.Strings(sortEnv)
		h.Write([]byte(strings.Join(sortEnv, "")))
	}

	// hash on the artifact manifest
	if artifact != nil {
		h.Write(artifact.RawManifest)
	}

	// hash on the filename, size and content of each input
	//
	// TODO: consider storing a map of filenames to hashes and cache based
	//       on the last modified time to reduce build time
	for _, input := range inputs {
		f, err := os.Open(input)
		if err != nil {
			return "", err
